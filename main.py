...
def exibe_simulacao(dados_bola, dados_robo):
    ...
    animation = FuncAnimation(fig=fig, func=update_data, frames=len(x_plot_r), repeat=False, interval=20)
...
def plota_graficos_posicao(dados_bola, dados_robo):
    ...
    plt.savefig("graficos/01_grafico_posicao.pdf")
    plt.close()
...
def plota_graficos_velocidade(dados_bola, dados_robo):
    ...
    plt.savefig("graficos/02_grafico_velocidade.pdf")
    plt.close()
...
def plota_grafico_dist_relativa(dados_robo):
    ...
    plt.savefig("graficos/04_grafico_dist_relativa.pdf")
...
def plota_grafico_aceleracao(dados_bola, dados_robo):
    ...
    eixo[0].set_xlabel("tempo (s)")
    eixo[1].set_xlabel("tempo (s)")
    eixo[0].set_ylabel("aceleração X (m/s²)")
    eixo[1].set_ylabel("aceleração Y (m/s²)")
    ...
    plt.savefig("graficos/03_grafico_aceleracao.pdf")
    plt.close()

def plota_grafico_traj_plano_xy(dados_bola, dados_robo):
    x_robo = []
    y_robo = []
    ...
    ax1.plot(x_bola, y_bola)
    ax1.plot(x_robo, y_robo)

    ax1.legend(['bola', 'robo'])
    ax1.set_xlabel('X')
    ax1.set_ylabel('Y')
    fig.suptitle('Trajetória no plano XY', fontsize=16)

    plt.savefig("graficos/00_grafico_traj_plano_xy.pdf")
...
def main():
    ...
    while True:
        ...
        print(f"A bola foi interceptada em {ponto_interceptacao['t']} segundos, nas coordenadas: X = {(ponto_interceptacao['x_bola']):.3g} e Y = {(ponto_interceptacao['y_bola']):.3g}")
        ...
        plota_grafico_traj_plano_xy(dados_bola, dados_interceptacao)
